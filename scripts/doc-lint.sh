#!/usr/bin/env bash
#
# doc-lint.sh — documentation anti-drift guard for the qbm-pgsql module.
#
...
# checks only qbm-pgsql's own Markdown (README.md, readme/**, CHANGELOG/SECURITY/CONTRIBUTING).
#
# Usage:  ./scripts/doc-lint.sh   (from the qbm-pgsql root)
#
set -uo pipefail
...
# project(qbm-pgsql VERSION ...) in CMakeLists.txt. That is a legitimate source rather than a
...
EXPECTED_VERSION="$(sed -n 's/^[[:space:]]*project(qbm-pgsql[[:space:]]\{1,\}VERSION[[:space:]]\{1,\}\([0-9][0-9.]*\)).*/\1/p' \
                    CMakeLists.txt 2>/dev/null | head -1)"
if [ -z "${EXPECTED_VERSION}" ]; then
  red "doc-lint: cannot read project(qbm-pgsql VERSION ...) from CMakeLists.txt"
...
if [ -n "${QB_CONFIG}" ]; then
  ...
  if [ "${_qbver}" != "${EXPECTED_VERSION}" ]; then
    red "doc-lint: lockstep drift -- CMakeLists.txt says qbm-pgsql ${EXPECTED_VERSION},"
...
FORBIDDEN='qb::Timestamp|qb::Duration|qb::TimePoint|to_timestamp\(|to_time_point\(|PgReply|qb::pg::detail::pgdate|qb::pg::detail::pgtime|qb::pg::detail::pgtimetz|tls_verify_peer'
...
while read -r f; do
  ...
  awk 'BEGIN{c=0} /^[[:space:]]*```/{c=!c; next} !c{print}' "$f" 2>/dev/null \
    ...
    [ ! -e "$r" ] && { red "  ${f} -> ${t} (missing)"; echo X >> /tmp/qbmpgsql-doclint-broken.$$; }
  done
done < <(doc_files)
broken=0; [ -f /tmp/qbmpgsql-doclint-broken.$$ ] && { broken=$(wc -l < /tmp/qbmpgsql-doclint-broken.$$); rm -f /tmp/qbmpgsql-doclint-broken.$$; }
...
while read -r f; do
  grep -oE 'https://github\.com/isndev/[A-Za-z0-9_.+-]+(/(blob|tree|raw)/[^/)" ]+)?' "$f" 2>/dev/null \
    ...
    case " ${ISNDEV_REPOS} " in
      *" ${repo} "*) ;;
      *) red "  ${f}: unknown repository 'isndev/${repo}' -> ${u}"; echo X >> /tmp/qbmpgsql-doclint-badurl.$$ ;;
    esac
    [ -z "${ref}" ] && continue
    [ "${ref}" = "main" ] && continue
    if [ "${#ref}" -eq 40 ]; then
    ...
    red "  ${f}: ref '${ref}' is not main or a permalink -> ${u}"; echo X >> /tmp/qbmpgsql-doclint-badurl.$$
  done
done < <(doc_files)
badurl=0; [ -f /tmp/qbmpgsql-doclint-badurl.$$ ] && { badurl=$(wc -l < /tmp/qbmpgsql-doclint-badurl.$$); rm -f /tmp/qbmpgsql-doclint-badurl.$$; }
...
while read -r f; do
  ...
  # Rightmost "qb <x.y.z>": this module's marker form is "qbm-pgsql @ qb 3.0.0", and "qbm"
