 ...
 * - (B&F) Both directions
 *
 * @note Same byte, different directions: PostgreSQL reuses type bytes for F vs B
 *       (e.g. `'C'` = Close from client, CommandComplete from server; `'D'` = Describe
 *       vs DataRow; `'E'` = Execute vs ErrorResponse; `'H'` = Flush vs CopyOutResponse).
 *       Routing uses direction: `message::frontend_tags()` vs `backend_tags()`, and
 *       incoming handlers only interpret **backend** meanings.
 *
 * @see https://www.postgresql.org/docs/current/protocol-message-formats.html
 */
enum message_tag {
...
public:
    ...
    /**
     * @brief Skip any unread payload bytes (safe no-op after full parse)
     *
     * Use for handlers that do not decode the full body (e.g. unsupported message types)
     * so invariants stay clear; framing already consumed the TCP message.
     */
