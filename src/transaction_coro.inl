...
pg_execute_void_sql(Transaction *self, std::string sql) {
    return pg_reply_awaiter<void>{
        [self, sql = std::move(sql)](pg_coro_complete<void> complete) mutable {
            self->execute(
                std::string_view(sql),
                [complete](Transaction &, resultset) mutable {
                    complete(::qb::pg::Reply<void>::success());
                },
                [complete](error::db_error const &e) mutable {
                    complete(::qb::pg::Reply<void>::failure(e));
                });
        }};
}
...
Transaction::execute(std::string_view expr) {
    return pg_reply_awaiter<resultset>{
        [this, sql = std::string(expr)](pg_coro_complete<resultset> complete) {
            this->execute(
                std::string_view(sql),
                [complete](Transaction &, resultset rs) mutable {
                    complete(::qb::pg::Reply<resultset>::success(rs.deep_snapshot()));
                },
                [complete](error::db_error const &e) mutable {
                    complete(::qb::pg::Reply<resultset>::failure(e));
                });
        }};
}
...
Transaction::prepare(std::string_view query_name, std::string_view expr, type_oid_sequence types) {
    return pg_reply_awaiter<PreparedQuery>{
        [this, qn = std::string(query_name), ex = std::string(expr),
         t = std::move(types)](pg_coro_complete<PreparedQuery> complete) mutable {
            this->prepare(
                ...
                [complete](error::db_error const &e) mutable {
                    complete(::qb::pg::Reply<PreparedQuery>::failure(e));
                });
        }};
}
...
Transaction::execute(std::string_view query_name, QueryParams &&params) {
    return pg_reply_awaiter<resultset>{
        [this, qn = std::string(query_name), p = std::move(params)](
            pg_coro_complete<resultset> complete) mutable {
            this->execute(
...
Transaction::savepoint(std::string_view name) {
    if (!pg_savepoint_name_ok(name))
        return pg_fail_resultset(error::client_error{
            "invalid savepoint name (use non-empty alphanumeric/underscore, max 63)"});
...
Transaction::execute_file(const std::filesystem::path& file_path) {
    return pg_reply_awaiter<resultset>{
        [this, path = file_path](pg_coro_complete<resultset> complete) mutable {
            try {
                if (!std::filesystem::exists(path)) {
                    complete(::qb::pg::Reply<resultset>::failure(
                        error::query_error("SQL file not found: " + path.string())));
                    return;
                }
                std::ifstream file(path);
                if (!file.is_open()) {
                    complete(::qb::pg::Reply<resultset>::failure(
                        error::query_error("Cannot open SQL file: " + path.string())));
                    return;
                }
                ...
                complete(::qb::pg::Reply<resultset>::failure(
                    error::query_error("Error reading SQL file: " + std::string(e.what()))));
            }
        }};
}
...
Transaction::prepare_file(std::string_view query_name, const std::filesystem::path& file_path,
    ...
    return pg_reply_awaiter<PreparedQuery>{
        [this, qn = std::string(query_name), path = file_path,
         t = std::move(types)](pg_coro_complete<PreparedQuery> complete) mutable {
            try {
                if (!std::filesystem::exists(path)) {
                    complete(::qb::pg::Reply<PreparedQuery>::failure(
                        error::query_error("SQL file not found: " + path.string())));
                    return;
                }
                std::ifstream file(path);
...
Transaction::notify(std::string_view channel, std::string_view payload) {
    return pg_try_build_and_execute_void_sql(this, [channel, payload] {
        return build_notify_sql(channel, payload);
    });
}

inline pg_reply_awaiter<void>
Transaction::listen(std::string_view channel) {
    return pg_try_build_and_execute_void_sql(this, [channel] {
        return build_listen_sql(channel);
    });
}
...
Transaction::unlisten(std::string_view channel) {
    return pg_try_build_and_execute_void_sql(this, [channel] {
        return build_unlisten_sql(channel);
    });
}
