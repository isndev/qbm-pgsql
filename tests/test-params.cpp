...
public:
    ParamSerializerModuleTest() : serializer_() {}
    
    // Test avec un vecteur de chaînes
    void test_string_vector(const std::vector<std::string>& values) {
        ...
        std::cout << "[TEST] Ajout d'un vecteur de " << values.size() << " chaînes comme paramètres\n";
    ...
    void debug_print_serializer() const {
    ...
    void verify_serializer(const std::vector<std::string>& expected_values) const {
        const std::vector<byte>& buffer = serializer_.params_buffer();
        const std::vector<integer>& types = serializer_.param_types();
