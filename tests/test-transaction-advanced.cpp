    ...
    auto status =
        db1_->begin(
                ...
                [&error_caught](error::db_error error) {
                    error_caught = true;
                    std::cout << "Transaction error callback triggered: " << error.what()
                              << std::endl;
                },
...
TEST_F(PostgreSQLAdvancedTransactionTest, TransactionTimeout) {
    ...
    auto status =
        db1_->begin([&timeout_operation_started](Transaction &t) {
                    ...
                    t.execute("INSERT INTO test_unique_constraint (unique_value) VALUES "
                              ...
                              [&setup_complete](Transaction &tr, results result) {
                                  ...
                                  tr.execute(
                                      ...
                                      [](Transaction &tr2, results result) {
                                          ASSERT_TRUE(false)
                                              << "Constraint violation should prevent "
                                                 "this callback";
                                      },
                                      [](error::db_error error) {
...
TEST_F(PostgreSQLAdvancedTransactionTest, TransactionWithCursor) {
    ...
    auto cursor_status =
        db1_->begin([&cursor_created, &fetch_executed, &rows_fetched,
                     FETCH_SIZE](Transaction &t) {
                // Declare a cursor
                t.execute(
