...
struct ScalarSmallint {
    ...
    static protocol_data_format
    expected_result_format() {
        return protocol_data_format::Binary;
    }
};
...
struct ScalarText {
    ...
    static cpp_t
    value() {
        return std::string{"unicode: \xC3\xA9\xC3\xA8\xE2\x82\xAC"};
    } // éè€
    static const char *
    col_type() {
        return "TEXT";
...
using ScalarTypes = ::testing::Types<ScalarSmallint, ScalarInteger, ScalarBigint, ScalarBigintMin, ScalarBoolean, ScalarText>;
...
TYPED_TEST(ScalarRoundTrip, BinaryParamTypedResult) {
    ...
    const std::string create = std::string("CREATE TEMP TABLE st (v ") + P::col_type() + ")";
    ASSERT_TRUE(db.execute(create, discard_query, discard_error).await());

    // Insert via typed binary parameter.
    ASSERT_TRUE(db.prepare("st_ins", "INSERT INTO st (v) VALUES ($1)", type_oid_sequence{}, discard_prepare, discard_error).await());
    ASSERT_TRUE(
        db.execute("st_ins", params(v), [](transaction &, results) {}, [](error::db_error e) { FAIL() << "insert: " << e.what(); }).await());

    // (a) Read back via prepared SELECT -> binary result column -> typed decode.
    ASSERT_TRUE(db.prepare("st_sel", "SELECT v FROM st", type_oid_sequence{}, discard_prepare, discard_error).await());
    bool decoded = false;
...
TEST_F(DataTypesRoundTrip, Float4_ExactValueAndNaNInf) {
    ASSERT_TRUE(db_->prepare("f4", "SELECT $1::float4", type_oid_sequence{oid::float4}, discard_prepare, discard_error).await());
    ...
    echo_f4(std::numeric_limits<float>::quiet_NaN(), [](float v) { EXPECT_TRUE(std::isnan(v)); });
    echo_f4(std::numeric_limits<float>::infinity(), [](float v) { EXPECT_TRUE(std::isinf(v) && v > 0); });
    echo_f4(-std::numeric_limits<float>::infinity(), [](float v) { EXPECT_TRUE(std::isinf(v) && v < 0); });
}

TEST_F(DataTypesRoundTrip, Float8_ExactValueAndNaNInf) {
    ASSERT_TRUE(db_->prepare("f8", "SELECT $1::float8", type_oid_sequence{oid::float8}, discard_prepare, discard_error).await());
    ...
    echo_f8(std::numeric_limits<double>::quiet_NaN(), [](double v) { EXPECT_TRUE(std::isnan(v)); });
    echo_f8(std::numeric_limits<double>::infinity(), [](double v) { EXPECT_TRUE(std::isinf(v) && v > 0); });
    echo_f8(-std::numeric_limits<double>::infinity(), [](double v) { EXPECT_TRUE(std::isinf(v) && v < 0); });
}
...
TEST_F(DataTypesRoundTrip, Bytea_RoundTripWithNul) {
    ASSERT_TRUE(db_->prepare("by", "SELECT $1::bytea", type_oid_sequence{oid::bytea}, discard_prepare, discard_error).await());
    const std::vector<byte> payload{
        static_cast<byte>(0xDE), static_cast<byte>(0xAD), static_cast<byte>(0x00), static_cast<byte>(0xBE), static_cast<byte>(0xEF)
    };
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "by", params(payload),
...
TEST_F(DataTypesRoundTrip, EmptyByteaParam_RoundTripsAsZeroLength) {
    ...
    ASSERT_TRUE(
        db_->prepare("eby", "SELECT length($1::bytea) AS n, $1::bytea AS b", type_oid_sequence{oid::bytea}, discard_prepare, discard_error)
            .await());
...
TEST_F(DataTypesRoundTrip, Uuid_RoundTrip) {
    ...
    ASSERT_TRUE(db_->prepare("uu", "SELECT $1::uuid", type_oid_sequence{oid::uuid}, discard_prepare, discard_error).await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "uu", params(u),
                       [&](transaction &, results r) {
...
TEST_F(DataTypesRoundTrip, Timestamp_FixedInstant_SecondTolerance) {
    const qb::wall_time ts = qb::wall_from_unix_seconds(kFixedUtcSeconds) + std::chrono::microseconds(789000);

    ASSERT_TRUE(db_->prepare("ts_ins", "SELECT $1::timestamp AS t", type_oid_sequence{oid::timestamp}, discard_prepare, discard_error).await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "ts_ins", params(ts),
                       [&](transaction &, results r) {
...
TEST_F(DataTypesRoundTrip, TimestampTZ_FixedInstant_SecondTolerance) {
    ...
    ASSERT_TRUE(
        db_->prepare("tz_ins", "SELECT $1::timestamptz AS t", type_oid_sequence{oid::timestamptz}, discard_prepare, discard_error).await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "tz_ins", params(ts),
                       [&](transaction &, results r) {
...
TEST_F(DataTypesRoundTrip, Numeric_ExactDecimal) {
    ASSERT_TRUE(db_->prepare("num", "SELECT $1::numeric AS n", type_oid_sequence{oid::numeric}, discard_prepare, discard_error).await());
...
TEST_F(DataTypesRoundTrip, ReadAsDoublePerColumnOidNotGarbage) {
    ASSERT_TRUE(db_->prepare("ad", "SELECT $1::numeric AS n, 42::int8 AS i, '2024-01-01'::date AS d", type_oid_sequence{oid::numeric},
...
TEST_F(DataTypesRoundTrip, ByteaWithLeadingNulsReadAsStringIsVerbatim) {
    ASSERT_TRUE(db_->prepare("bnul", "SELECT $1::bytea AS b", type_oid_sequence{oid::bytea}, discard_prepare, discard_error).await());
    ...
    ASSERT_TRUE(db_->execute(
                       ...
                       [&](transaction &, results r) {
                           ...
                           ASSERT_FALSE(r[0][0].is_null());
                           EXPECT_EQ(r.field(0).format_code, protocol_data_format::Binary);
                           EXPECT_EQ(r[0][0].as<std::string>(), std::string("\0\0\x01\x02hi", 6)); // 6 bytes verbatim
                           ok = true;
                       },
                       [](error::db_error e) { FAIL() << e.what(); })
                    .await());
...
TEST_F(DataTypesRoundTrip, Numeric_HighPrecision_RealColumn) {
    ...
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE hp (n NUMERIC)", discard_query, discard_error).await());
    ASSERT_TRUE(
        db_->prepare("hp_ins", "INSERT INTO hp (n) VALUES ($1)", type_oid_sequence{oid::numeric}, discard_prepare, discard_error).await());
    ASSERT_TRUE(db_->prepare("hp_sel", "SELECT n FROM hp", type_oid_sequence{}, discard_prepare, discard_error).await());

    const std::string big = "999999999999999999999999999999.99999999999999999999";
    ASSERT_TRUE(
        db_->execute(
               "hp_ins", params(numeric(big)), [](transaction &, results) {}, [](error::db_error e) { FAIL() << "insert: " << e.what(); })
            .await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "hp_sel", params(),
...
TEST_F(DataTypesRoundTrip, Numeric_Specials_NaNAndInfinity) {
    ...
    ASSERT_TRUE(db_->prepare("nspec", "SELECT $1::numeric AS n", type_oid_sequence{oid::text}, discard_prepare, discard_error).await());
    struct Case {
        const char *in;
        const char *out;
    };
    for (const auto &c : {Case{"NaN", "NaN"}, Case{"Infinity", "Infinity"}, Case{"-Infinity", "-Infinity"}}) {
        bool ok = false;
        ASSERT_TRUE(db_->execute(
                           ...
                           [&](transaction &, results r) {
                               ASSERT_EQ(r.size(), 1u);
                               EXPECT_EQ(r.field(0).format_code, protocol_data_format::Binary);
                               EXPECT_EQ(r[0][0].as<numeric>().str(), c.out) << "for NUMERIC special " << c.in;
                               ok = true;
                           },
                           [&](error::db_error e) {
...
TEST_F(DataTypesRoundTrip, Date_RoundTripThroughRealColumn) {
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE dt (d DATE)", discard_query, discard_error).await());
    ASSERT_TRUE(db_->prepare("dt_ins", "INSERT INTO dt (d) VALUES ($1)", type_oid_sequence{oid::date}, discard_prepare, discard_error).await());
    ASSERT_TRUE(db_->prepare("dt_sel", "SELECT d FROM dt ORDER BY d", type_oid_sequence{}, discard_prepare, discard_error).await());

    for (const char *iso : {"1970-01-01", "2050-12-31"}) {
        ASSERT_TRUE(db_->execute(
                           "dt_ins", params(qb::date::parse(iso).value()), [](transaction &, results) {},
...
TEST_F(DataTypesRoundTrip, Date_InfinitySentinels_RenderAsText) {
    ...
    ASSERT_TRUE(db_->prepare("dinf", "SELECT $1::date::text AS d", type_oid_sequence{oid::text}, discard_prepare, discard_error).await());
...
TEST_F(DataTypesRoundTrip, IntArray_OneDimensional) {
    ASSERT_TRUE(db_->prepare("ia", "SELECT $1::int[] AS a", type_oid_sequence{oid::int4_array}, discard_prepare, discard_error).await());
...
TEST_F(DataTypesRoundTrip, NumericArrayParam_PreviouslyAnyarray_BindsAndMatches) {
    ASSERT_TRUE(db_->prepare("nap", "SELECT $1::numeric[] = ARRAY[1.5,2.5,3.25]::numeric[] AS eq", type_oid_sequence{oid::numeric_array},
                             discard_prepare, discard_error)
                    .await());
...
TEST_F(DataTypesRoundTrip, DateTimeNumericIntArray_BinaryRoundTrip) {
    ...
    ASSERT_TRUE(db_->prepare("dtna_ins", "INSERT INTO dtna_test (d, t, n, a) VALUES ($1, $2, $3, $4)",
                    ...
                    .await());
    ASSERT_TRUE(db_->prepare("dtna_sel", "SELECT d, t, n, a FROM dtna_test", type_oid_sequence{}, discard_prepare, discard_error).await());
    ...
    ASSERT_TRUE(
        db_->execute(
               "dtna_ins", params(d, t, n, a), [](transaction &, results) {}, [](error::db_error e) { FAIL() << "insert: " << e.what(); })
            .await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "dtna_sel", params(),
...
TEST_F(DataTypesRoundTrip, IntervalTimetzArrays_BinaryRoundTrip) {
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE ita_test AS SELECT "
                    ...
                    .await());
    ASSERT_TRUE(
        db_->prepare("ita_sel", "SELECT iv, tz, ba, fa, bo FROM ita_test", type_oid_sequence{}, discard_prepare, discard_error).await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "ita_sel", params(),
                       [&](transaction &, results r) {
...
TEST_F(DataTypesRoundTrip, NetworkBitTypes_RouteAsText) {
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE nbt_test AS SELECT "
                    ...
                    .await());
    ASSERT_TRUE(
        db_->prepare("nbt_sel", "SELECT a, c, m, b, v, p, r, l FROM nbt_test", type_oid_sequence{}, discard_prepare, discard_error).await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "nbt_sel", params(),
                       [&](transaction &, results r) {
...
TEST_F(DataTypesRoundTrip, QbCivilTypes_BinaryRoundTrip) {
    ...
    ASSERT_TRUE(db_->prepare("qct_ins", "INSERT INTO qct_test (d, t, z, iv) VALUES ($1, $2, $3, $4)",
                    ...
                    .await());
    ASSERT_TRUE(db_->prepare("qct_sel", "SELECT d, t, z, iv FROM qct_test", type_oid_sequence{}, discard_prepare, discard_error).await());
    ...
    ASSERT_TRUE(
        db_->execute(
               "qct_ins", params(d, t, z, iv), [](transaction &, results) {}, [](error::db_error e) { FAIL() << "insert: " << e.what(); })
            .await());
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                       "qct_sel", params(),
...
TEST_F(DataTypesRoundTrip, Json_RoundTripPreservesStructure) {
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE j_test (j JSON)", discard_query, discard_error).await());
    // NB: j_test has only the `j` column — a `RETURNING id` here is 42703 (no such column).
    ASSERT_TRUE(
        db_->prepare("j_ins", "INSERT INTO j_test (j) VALUES ($1)", type_oid_sequence{oid::json}, discard_prepare, discard_error).await());
    ...
    ASSERT_TRUE(
        db_->execute("j_ins", params(in), [](transaction &, results) {}, [](error::db_error e) { FAIL() << "insert: " << e.what(); }).await());
...
TEST_F(DataTypesRoundTrip, Jsonb_CanonicalShape) {
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE jb_test (jb JSONB)", discard_query, discard_error).await());
    ASSERT_TRUE(
...
TEST_F(DataTypesRoundTrip, Jsonb_Containment_Operator) {
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE jbc (jb JSONB)", discard_query, discard_error).await());
    ASSERT_TRUE(db_->execute(R"(INSERT INTO jbc (jb) VALUES ('{"a":1,"b":2}'::jsonb))", discard_query, discard_error).await());
    ASSERT_TRUE(
        db_->prepare("jbc_sel", "SELECT count(*)::int FROM jbc WHERE jb @> $1", type_oid_sequence{oid::jsonb}, discard_prepare, discard_error)
            .await());
...
TEST_F(DataTypesRoundTrip, ResultSet_JsonProjection) {
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE rj (id INT, s SMALLINT, t TEXT, b BOOLEAN, n TEXT)", discard_query, discard_error).await());
    ASSERT_TRUE(db_->execute("INSERT INTO rj VALUES (1,123,'Text value',true,NULL),"
...
TEST_F(DataTypesRoundTrip, IntegerRoundTrip_CallbackAndCoroutine) {
    ...
    ASSERT_TRUE(db_->execute("CREATE TEMP TABLE rt (v INT)", discard_query, discard_error).await());
    ASSERT_TRUE(db_->prepare("rt_ins", "INSERT INTO rt (v) VALUES ($1)", type_oid_sequence{oid::int4}, discard_prepare, discard_error).await());
    ASSERT_TRUE(db_->execute("rt_ins", params(expected), [](transaction &, results) {}, [](error::db_error e) { FAIL() << e.what(); }).await());
