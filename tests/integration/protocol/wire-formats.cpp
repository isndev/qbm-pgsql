...
protected:
    ...
    SetUp() override {
        ...
        ASSERT_TRUE(db_->execute("CREATE TEMP TABLE qb_copy_t (v TEXT NOT NULL)",
...
TEST_F(WireFormats, Prepared_IntResultColumnIsBinary) {
    ASSERT_TRUE(db_->prepare("bin_one", "SELECT ($1::int * 2) AS n",
...
TEST_F(WireFormats, Prepared_ResultFormatMatrix) {
    ASSERT_TRUE(db_->prepare(
                      ...
                      type_oid_sequence{oid::int8, oid::text, oid::float8, oid::text,
                                        oid::text, oid::uuid, oid::bytea, oid::text},
    ...
    ASSERT_TRUE(db_->execute(
                      "fmt_matrix",
                      params{static_cast<int64_t>(-9), std::string{"pq"}, 1.25,
                             std::string{"3.50"}, std::string{"2023-01-15 12:34:56+00"},
                             u, bin, std::string{"192.168.0.1"}},
                      [&](transaction &, results r) {
                          ASSERT_EQ(r.size(), 1u);
...
TEST_F(WireFormats, Prepared_ParameterFormat_BinaryAndText) {
    ...
    bool ok = false;
    ASSERT_TRUE(db_->execute(
                      "param_fmt", params{static_cast<int64_t>(123456789012345LL),
                                          std::string{"hi"}},
                      [&](transaction &, results r) {
                          ASSERT_EQ(r.size(), 1u);
...
TEST_F(WireFormats, Prepared_TextParameter_TextResult) {
    ASSERT_TRUE(db_->prepare("bin_textparam", "SELECT $1::text AS s",
...
TEST_F(WireFormats, Prepared_NoParameters_StillBinaryResults) {
    ASSERT_TRUE(db_->prepare("bin_nop", "SELECT true AS b, 42::int AS n",
...
TEST_F(WireFormats, Prepared_JsonText_JsonbBinary) {
    ASSERT_TRUE(db_->prepare("json_col", "SELECT '[]'::json AS j",
                             type_oid_sequence{}, discard_prepare, discard_error)
                    .await());
    ASSERT_TRUE(db_->prepare("jsonb_col", "SELECT '[true,1]'::jsonb AS jb",
                             type_oid_sequence{}, discard_prepare, discard_error)
                    .await());
    bool json_ok = false;
    ASSERT_TRUE(db_->execute(
...
TEST_F(WireFormats, CopyInStream_LoadsRowsThenSelectableBack) {
    ...
    qb::io::async::run_sync([&]() -> qb::io::async::task<void> {
        int  step  = 0;
        auto reply = co_await db_->copy_in(
            "COPY qb_copy_t (v) FROM STDIN",
            [&]() -> std::optional<std::string> {
                switch (step++) {
                    case 0: return std::string{"row1\n"};
                    case 1: return std::string{"row2\nrow3\n"};
...
TEST_F(WireFormats, CopyFromStdin_NoSource_RejectedThenRecovers) {
    ...
    auto st        = db_->execute(
                            "COPY qb_copy_t FROM STDIN",
                            [](transaction &, results) {
                                FAIL() << "COPY FROM STDIN must not complete as a normal query";
                            },
...
TEST_F(WireFormats, CopyToStdout_TextFormat_StreamsExpectedBytes) {
    ...
    std::string copy_err;
    qb::io::async::run_sync([&]() -> qb::io::async::task<void> {
        auto reply = co_await db_->copy_out(
...
TEST_F(WireFormats, EmptySimpleQuery_Completes) {
    bool ok = false;
    auto st = db_->execute(
                     "", [&ok](transaction &, results) { ok = true; },
                     [](error::db_error const &e) { FAIL() << "empty query: " << e.what(); })
                  .await();
    EXPECT_TRUE(st);
    EXPECT_TRUE(ok);
}
