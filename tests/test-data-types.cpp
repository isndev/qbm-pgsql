...
protected:
    ...
    std::vector<qb::pg::byte> createCorruptedBuffer(size_t size) {
    ...
    template<typename T>
    std::vector<qb::pg::byte> createWrongSizeBuffer(T value, size_t wrong_size) {
...
TEST_F(PostgreSQLDataTypesTest, NonStandardSizeBuffers) {
    ...
    std::vector<byte> wrong_size_buffer = {0x30, 0x39, static_cast<char>(0xFF)}; // smallint (12345) avec un octet supplémentaire
...
TEST_F(PostgreSQLDataTypesTest, CorruptedData) {
    ...
    std::vector<byte> corrupted_smallint = {static_cast<char>(0xFF), static_cast<char>(0xFF)}; // Valeur qui peut causer des problèmes
