...
protected:
    void SetUp() override {
        ...
        auto status = db_->execute("CREATE TEMP TABLE test_transactions (id SERIAL PRIMARY KEY, value TEXT)")
                .await();
        ASSERT_TRUE(status);
    }
    ...
    void TearDown() override {
        if (db_) {
            auto status = db_->execute("DROP TABLE IF EXISTS test_transactions")
                    .await();
            ASSERT_TRUE(status);
            db_->disconnect();
            db_.reset();
...
TEST_F(PostgreSQLTransactionTest, BasicTransaction) {
    ...
    auto status = db_->begin(
        [&success](Transaction& t) {
            t.execute("SELECT * FROM test_transactions", 
...
TEST_F(PostgreSQLTransactionTest, TransactionRollback) {
    ...
    auto status = db_->begin(
        [](Transaction& t) {
            t.execute("INSERT INTO nonexistent (value) VALUES ('test')", 
...
TEST_F(PostgreSQLTransactionTest, NestedTransactions) {
    ...
    auto status = db_->begin(
        [&success](Transaction& t) {
            t.savepoint("nested_sp", 
                [&success](Transaction& tr) {
                    tr.execute("INSERT INTO test_transactions (value) VALUES ('test')", 
...
TEST_F(PostgreSQLTransactionTest, DISABLED_SavepointRollback) {
    ...
    auto status = db_->begin(
        [&before_savepoint, &in_savepoint, &after_savepoint](Transaction& t) {
            t.execute("INSERT INTO test_transactions (value) VALUES ('before_savepoint')",
                     [&before_savepoint, &in_savepoint, &after_savepoint](Transaction& tr, results result) {
                         before_savepoint = true;
                         tr.savepoint("sp1",
                             [&in_savepoint](Transaction& tr2) {
                                 tr2.execute("INSERT INTO test_transactions (value) VALUES ('in_savepoint')",
                                          [&in_savepoint](Transaction& tr3, results result) {
                                              in_savepoint = true;
                                              // Forcer une erreur dans le savepoint
                                              tr3.execute("SELECT nonexistent",
                         ...
                         tr.execute("SELECT * FROM test_transactions WHERE value = 'before_savepoint'",
    ...
    auto status2 = db_->begin(
        [](Transaction& t) {
            t.execute("SELECT * FROM test_transactions WHERE value = 'in_savepoint'",
                     [](Transaction& tr, results result) {
                         ASSERT_EQ(result.size(), 0);
                     },
                     [](error::db_error error) {
            ...
            t.execute("SELECT * FROM test_transactions WHERE value = 'before_savepoint'",
                     [](Transaction& tr, results result) {
                         ASSERT_EQ(result.size(), 1);
                     },
                     [](error::db_error error) {
        ...
        [&success1, &success2](Transaction& t) {
            t.savepoint("sp1",
                [&success1, &success2](Transaction& tr1) {
                    tr1.execute("INSERT INTO test_transactions (value) VALUES ('sp1')",
                             [&success1, &success2](Transaction& tr2, results result) {
                                 success1 = true;
                                 tr2.savepoint("sp2",
                                     [&success2](Transaction& tr3) {
                                         tr3.execute("INSERT INTO test_transactions (value) VALUES ('sp2')",
...
TEST_F(PostgreSQLTransactionTest, CommitMultipleChanges) {
    ...
    auto status = db_->begin(
        [&insert_success, &select_success](Transaction& t) {
            t.execute("INSERT INTO test_transactions (value) VALUES ('test1')",
                     [&insert_success, &select_success](Transaction& tr1, results result) {
                         insert_success = true;
                         tr1.execute("INSERT INTO test_transactions (value) VALUES ('test2')",
                                  [&select_success](Transaction& tr2, results result) {
                                      tr2.execute("SELECT * FROM test_transactions",
