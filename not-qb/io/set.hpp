...
struct protocol_parser<std::set<T>, TEXT_DATA_FORMAT>
    ...
    typedef detail::text_container_parser<protocol_parser<std::set<T>, TEXT_DATA_FORMAT>,
                                          std::set<T>>
        base_type;
    typedef typename base_type::value_type value_type;
    ...
    operator()(InputIterator begin, InputIterator end) {
        ...
        return base_type::parse(
            begin, end, std::inserter(base_type::value, base_type::value.begin()));
    }
};
